...
class BingImagesSpider:
    ...
    def run(self):
        ...
        for i in range(int(self.amount / self.per_page_images * 1.5) + 1):  # 由于有些图片会重复，故先请求1.5倍图片，豁免
            url = BingImagesSpider.bing_image_url_pattern.format(
                self.keyword,
                i * self.per_page_images,
                self.per_page_images
            )
        ...
        # 删除重复的图片，避免重复下载
        info_list = deduplication(info_list)
        ...
        # 打印所有图片名称及其对应链接
        # for info in info_list:
        #     print(f"图片名称: {info['image_title']}, 图片链接: {info['image_url']}")
...
if __name__ == '__main__':
    ...
    # 需要的图片数量：100
    start = time()
    data = BingImagesSpider('TCP/IP模型图', 10).run()
    print(data)
    # print(time() - start)
