...
from pdf_generator import generate_content_and_pdf  # 导入函数
import os
...
async def generate_teaching_design_endpoint(request: TeachingDesignRequest):
    ...
    pdf_urls = generate_content_and_pdf([teaching_design], ["教学设计"], ["teaching_design.pdf"])
...
async def generate_exercises_endpoint(request: ExerciseRequest):
    ...
    # 分别生成练习题和答案解析的 PDF 文件
    pdf_urls = generate_content_and_pdf(
...
async def generate_markdown_endpoint(request: ExerciseRequest):
    api_key = config['api_keys']['tongyi_api_key']
    teaching_design = generate_teaching_design(
        request.subject, request.topic, request.goals,
        request.duration, request.grade, api_key
    ...
    markdown_content = {
        "teaching_design": f"# 教学设计\n\n{teaching_design}",
        "exercises": f"# 练习题\n\n{exercises}",
        "answers_and_explanations": f"# 答案和解析\n\n{answers_and_explanations}"
    }
